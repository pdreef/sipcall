...
int main(int argc, char *argv[])
{
	...
	if (!app_cfg.sip_domain || !app_cfg.sip_user || !app_cfg.sip_password || !app_cfg.phone_number || (!app_cfg.tts && !app_cfg.tts_file) )
	{
...
static void usage(int error)
{
	...
	puts  ("  -tts=string   Text to speak (if not supplied, play-back file -ttsf)");
	puts  ("  -ttsf=string  TTS speech file name to save text or playback (if not supplied, use play.wav)");
